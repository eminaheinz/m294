...
function LaenderTable() {
        ...
        return(
            ...
            {laender.map((row) => {
                ...
                <tr key={row.id}>
                    ...
                    <td><Link className="btn btn-info" to={`/laender/edit/${row.id}`}><Pencil color="white" size={15} /></Link></td>
    ...
    const deleteData = async () => {
        ...
        /* Fehler abfangen */
        try {
            const response = await axios.delete("https://emina.dnet.ch/laender/" + modalID);
            console.log(modalID)
            removeDataFromList();
    ...
    function renderModal(){
        return(<Modal show={show} onHide={() => handleShow(false)}>
            ...
            <Modal.Body>Wollen Sie den ausgewählten Land wirklich löschen?</Modal.Body>
            <Modal.Footer>
