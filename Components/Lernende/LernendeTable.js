...
function LernendeTable() {
        ...
        return(
            ...
            {lernende.map((row) => {
                ...
                <tr key={row.id}>
                    ...
                    <td><Link className="btn btn-info" to={`/lernende/edit/${row.id}`}><Pencil color="white" size={15} /></Link></td>
    ...
    const deleteData = async () => {
        ...
        /* Fehler abfangen */
        try {
            const response = await axios.delete("https://emina.dnet.ch/lernende/" + modalID);
            console.log(modalID)
            removeDataFromList();
