...
function DozentenTable() {
        ...
        return(
            ...
            {dozenten.map((row) => {
                ...
                <tr key={row.id}>
                    ...
                    <td><Link className="btn btn-info" to={`/dozenten/edit/${row.id}`}><Pencil color="white" size={15} /></Link></td>
    ...
    const deleteData = async () => {
        ...
        /* Fehler abfangen */
        try {
            const response = await axios.delete("https://emina.dnet.ch/dozenten/" + modalID);
            console.log(modalID)
            removeDataFromList();
