...
function KurseShowForm() {
        ...
        return(
            ...
            {kurse.map((row) => {
                ...
                <tr key={row.ll_id}>
                    ...
                    <td><Link className="btn btn-info" to={`/lehrbetriebe/show/AddStartEndBeruf/${row.ll_id}`}><Pencil color="white" size={15} /></Link></td>
    ...
    const deleteData = async () => {
        ...
        /* Fehler abfangen */
        try {
            const response = await axios.delete("https://emina.dnet.ch/lehrbetriebe_lernende/" + modalID);
    ...
    return(
        <div>
            <h1>Lernende in diesem Kurs<Link className="btn btn-primary" to={`LehrbetriebLernenderAdd/`}>Lernende Hinzufügen <PlusCircle color="white" size={15} /></Link></h1>
